...
// Quiz dengan pilihan ganda (ABCD)
const quizLevels = {
  easy: [
    {
    ...
    },
    {
      ...
      choices: ["172.16.0.1", "10.0.0.1", "192.168.1.1", "255.255.0.1"],
      a: "172.16.0.1"
    },
    {
      q: "Contoh IP Address kelas C adalah?",
      choices: ["192.168.1.1", "172.16.0.1", "10.0.0.1", "255.255.255.255"],
      a: "192.168.1.1"
    },
    {
      q: "Berapa rentang IP kelas A?",
      choices: ["1.0.0.0 – 126.255.255.255", "128.0.0.0 – 191.255.255.255", "192.168.0.0 – 192.168.255.255", "224.0.0.0 – 239.255.255.255"],
    ...
    },
    {
      ...
      choices: ["Default Gateway", "Loopback atau localhost", "Broadcast", "Multicast"],
  ...
  medium: [
    {
      ...
      choices: ["254 host", "256 host", "512 host", "128 host"],
      a: "254 host"
    },
    {
    ...
    },
    {
      ...
      choices: ["192.168.10.0", "192.168.10.1", "192.168.10.255", "192.168.10.128"],
      a: "192.168.10.0"
    },
    {
      q: "IP 192.168.10.1/24 memiliki broadcast address apa?",
      choices: ["192.168.10.255", "192.168.10.0", "192.168.10.128", "192.168.10.1"],
      a: "192.168.10.255"
    },
    {
    ...
    },
    {
      ...
      choices: ["192.168.1.128/25", "192.168.1.0/25", "192.168.1.64/25", "192.168.1.192/25"],
      a: "192.168.1.128/25"
    },
    {
    ...
    },
    {
      ...
      choices: ["16.777.214 host", "16.777.216 host", "65.534 host", "254 host"],
      a: "16.777.214 host"
    },
    {
...
document.getElementById('mentorSendBtn').addEventListener('click', () => {
  ...
  setTimeout(() => {
    ...
    if (input.includes("subnet mask")) {
      response = "Subnet mask adalah pembagi antara bagian jaringan (network) dan bagian perangkat (host) ...";
    } else if (input.includes("ip address")) {
      response = "IP Address adalah alamat unik untuk tiap perangkat dalam jaringan komputer...";
    } else if (input.includes("host")) {
      response = "Host dalam konteks jaringan ialah perangkat akhir (komputer, ponsel, printer)...";
    } else {
      response = "Pertanyaan menarik! Bisa kamu spesifikasikan sedikit: misalnya ‘bagaimana cara menghitung jumlah host?’, atau ‘apa itu broadcast address?’.";
    }
